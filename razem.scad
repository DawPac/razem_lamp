...
module pin_shape(i) {
    minkowski() {
        offset(delta = -pin_inset) foot(i);
        square([0.01, 2 * pin_inset], center = true);
    }
}
module korytko(i) {
...
module litera(i) {
    difference() {
        union() {
            ...
            // czop poniżej płaszczyzny oparcia: wcięty w X, pełny w Y
            translate([0, 0, -pin_len])
                linear_extrude(height = pin_len + 0.01)
                    pin_shape(i);
            // domknięcie nad płaszczyzną: pełna stopa przez denko (rant oparcia)
            linear_extrude(height = text_wall + przelot_over)
                foot(i);
        }
        // kanał: przez czop i denko, do wnętrza litery
        translate([0, 0, -pin_len - 1])
            linear_extrude(height = pin_len + text_wall + przelot_over + 2)
                offset(delta = -pin_wall) pin_shape(i);
    }
}
...
module podstawka() {
    ...
    difference() {
        ...
        for (i = [0 : len(text) - 1])
            translate([0, 0, -pin_len])
                linear_extrude(height = pin_len + 0.01)
                    offset(delta = pin_clearance) pin_shape(i);
    }
}
...
if (to_render == "all")
    for (i = [0 : len(text) - 1])
        translate([0, -explode, 0]) pokrywka_stoja(i);to_render = "all"; // [text, lid, base, all]
